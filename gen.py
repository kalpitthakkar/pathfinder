...
if __name__ == '__main__':
    ...
    parser.add_argument('-sp', '--splits', nargs='*', default=['curv_baseline', 'curv_contour_length_9', 'curv_contour_length_14'], help='Splits with the samples \
        ...
        while not complete:
            for i,fperm in enumerate(finger_assign_perms):
                for j,lperm in enumerate(level_order_perms):
                    ...
                    try:
                        curs.execute("""INSERT INTO pfind_exp_settings VALUES (%s,%s,%s,%s,%s)""", (j+1,i+1,sub_itr+1,csvfile,speed))
                        conn.commit()
                    except:
